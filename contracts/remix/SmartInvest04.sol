...
contract SmartInvest04 is ISmartInvest04, Secure {
  ...
  uint256 private constant bnbPrice = 415_00_000_000;
  ...
  function calcPercent(uint256 value) internal view returns (uint256) {
    return value.mul(MINIMUM_PERCENT).div(MINIMUM_INVEST);
  }
  ...
  function rewardPercent(uint256 value) public view override returns (uint256) {
    if (value < HUNDRED_DOLLARS) return value.mul(BASE_REWARD_PERCENT).div(HUNDRED_DOLLARS);
    uint256 percent = BASE_REWARD_PERCENT.add(calcPercent(value.sub(MINIMUM_INVEST)));
  ...
  function bnbToUSD(uint256 value) public pure override returns (uint256) {
    return value.mul(bnbPrice).div(10**18);
  }

  function USDToBnb(uint256 value) public pure override returns (uint256) {
